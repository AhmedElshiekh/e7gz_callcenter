...
/*  {
    ...
    // getCurrentDate();
    // currentDate = _allDoctorsController.currentDate;
    // update();
    print('$LOGD date  = $currentDate');
    getAvailableTime(_allDoctorsController.currentDate,docKey);
    update();
  }*/
  ...
  Future<List<AvailableTimesPerDayData>>getAvailableTime(date,docKey) async{
    ...
    try {
      ...
      print(
          '$LOGD getAvailableTime:Json: ${docAvailableTimesPerDay.toJson()} -----> ${docAvailableTimesPerDay.data.length} ');
      if (docAvailableTimesPerDay.status ) {
        ...
        print('$LOGD getAvailableTime length::: ${docAvailableTimesPerDay.length}');
        print('$LOGD getAvailableTime:Res:: ${availableTimes.length}');
        modalHudController.changeisLoading(false);
        update();
  ...
  getCurrentDate()async{
    ...
    update();
    // getAvailableTime(currentDate,docKey);
  }
  ...
  Future<void> selectDate(context) async {
    ...
    if (pickedDate != null){
      ...
      update();
      getAvailableTime(formatter.format(pickedDate),docKey);
    }else{
      selectedWeekDay =  getWeekDay(now.weekday);
